    ...
    <>
      ...
      <div style={{ border: '2px solid #14110D', background: '#fff', marginBottom: '32px' }}>
        <div style={{ padding: '16px 20px', borderBottom: '2px solid #14110D', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {/* Total di header sudah dihapus dari sini */}
        </div>
        ...
        {salesRows.map((r, idx) => (
          <div key={idx} style={{ display: 'grid', gridTemplateColumns: gridTable1, gap: '16px', padding: '14px 20px', borderBottom: '1px solid #ddd5c4', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right', fontFamily: "'Space Mono', monospace", fontSize: '13px' }}>
              {rp(r.price)}
            </div>
            
            <div style={{ textAlign: 'right', fontFamily: "'Space Mono', monospace", fontSize: '13px', fontWeight: 700 }}>
              {r.units}
            </div>
            
            <div style={{ textAlign: 'right', fontFamily: "'Space Mono', monospace", fontSize: '13px', fontWeight: 700, color: '#1f7a3d' }}>
              {rp(r.revenue)}
            </div>
          </div>
        ))}

        {/* --- BARIS TOTAL (Baru ditambahkan di sini) --- */}
        <div style={{ display: 'grid', gridTemplateColumns: gridTable1, gap: '16px', padding: '16px 20px', background: '#f7f4ee', borderTop: '2px solid #14110D', alignItems: 'center' }}>
          <div style={{ fontFamily: "'Archivo'", fontWeight: 900, fontSize: '16px', textTransform: 'uppercase' }}>
            TOTAL
          </div>
          
          {/* Kolom Tipe dan Harga dikosongkan (hanya div kosong) */}
          <div></div>
          <div></div>
          
          <div style={{ textAlign: 'right', fontFamily: "'Space Mono', monospace", fontSize: '14px', fontWeight: 700 }}>
            {totalUnits} unit
          </div>
          
          <div style={{ textAlign: 'right', fontFamily: "'Space Mono', monospace", fontSize: '15px', fontWeight: 900, color: '#1f7a3d' }}>
            {rp(totalRevenue)}
          </div>
        </div>
      </div>

      {/* --- SECTION 2: SEMUA PESANAN --- */}
      <div style={{ border: '2px solid #14110D', background: '#fff' }}>
        ...
        {data.orders.map((o, idx) => (
          <div key={idx} style={{ display: 'grid', gridTemplateColumns: gridTable2, gap: '16px', padding: '14px 20px', borderBottom: '1px solid #ddd5c4', alignItems: 'center' }}>
            ...
            <span style={{ textAlign: 'right', fontFamily: "'Space Mono', monospace", fontSize: '13px', fontWeight: 700 }}>
              {rp(o.total)}
            </span>
